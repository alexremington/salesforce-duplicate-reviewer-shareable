...
The staging source report is Contact-only and does not expose `Mirror Of` in the report metadata or CSV headers. Any Contact-level mirror field must therefore be supplied by a companion source or post-export normalization step before Duplicate Reviewer matching starts.
