...
- For `low` or `medium` work, switch to `gpt-5.4 mini` when the surface allows model selection and say that this requirement was applied.
- For `high` work, stop after the warning and wait for explicit user approval before design iteration, implementation, or broad validation begins.
...
- If any one of these is missing, the work is not allowed to proceed: cost label, Hume-first artifact when required, `gpt-5.4 mini` switch when applicable, or explicit approval for `high`-cost work.
