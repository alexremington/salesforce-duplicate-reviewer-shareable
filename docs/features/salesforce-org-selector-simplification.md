...
- Data source: Local Salesforce org preferences and the currently loaded dataset source metadata.
- Object type: Applies to both Contact and Account workflows.
- Primary actions: Select a recent org, apply the org, and see mismatch status update.
...
- Data writes that are not allowed: No new server-side writes are introduced by the selector simplification.
...
- Manual checks: open the `Source` rail, compare recent orgs, and confirm the selected org details remain readable without freeform editing.
