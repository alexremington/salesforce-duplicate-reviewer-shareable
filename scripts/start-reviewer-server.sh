...
server_supports_required_features() {
  local health
  health="$(server_health)"
  [[ "${health}" == *'"salesforceMerge":true'* && "${health}" == *'"latestStagingFiles":true'* && "${health}" == *'"jsonDatasets":true'* && "${health}" == *'"staticAssetRoot":true'* && "${health}" == *'"svgStaticAssets":true'* && "${health}" == *'"brandHeaderVersion":"shared-logo-contact-v1"'* ]]
}
