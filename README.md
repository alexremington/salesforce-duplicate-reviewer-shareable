...
For the prod Contacts handoff, use `scripts/run-prod-contacts-bulk-query.sh` to run the prod pull and open Duplicate Reviewer directly on the `prod-contacts` autoload route. The prod launcher keeps its latest output separate from staging so the two recent-file entries do not collide.
